    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-6 font-sans transition-colors">
      ...
      <div className="flex items-start justify-between mb-7">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Track your progress and identify patterns</p>
        </div>
        <select
          value={range}
      ...
      <div className="grid grid-cols-4 gap-4 mb-5">
        {stats.map((s) => (
          <div key={s.label} className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm flex items-start gap-4">
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "1.9fr 1fr" }}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm">
          ...
          {topHabits.length === 0 ? (
            <p className="text-xs text-gray-400 dark:text-gray-500 pt-2">No habits found.</p>
          ) : (
            topHabits.map((h) => (
              <div key={h.id} className="flex items-center gap-3 py-2.5 border-t border-gray-100 dark:border-gray-700">
                <span className="w-5 h-5 rounded-md bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-xs font-bold text-indigo-500 dark:text-indigo-300 shrink-0">
                  {h.rank}
                </span>
                <span className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-lg shrink-0">
                  {h.icon}
                </span>
                <div className="flex min-w-0 align-center justify-between gap-2">
                  ...
                  <span className="inline-flex items-center gap-1 text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-md mt-0.5">
